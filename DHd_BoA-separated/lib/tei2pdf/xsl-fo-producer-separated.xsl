...
<!-- Stagger the pages for print but not PDF -->
  ...
  <xsl:template name="publisher_info">
    <xsl:attribute name="margin-top">5.5in</xsl:attribute>
    <xsl:attribute name="font-family"><xsl:value-of select="$main_font"/></xsl:attribute>
  </xsl:template>
  ...
  <xsl:template name="intro_head_large_center">
    ...
    <xsl:attribute name="text-align">center</xsl:attribute>
    <xsl:attribute name="space-before">.8in</xsl:attribute>
    <xsl:attribute name="space-after">.2in</xsl:attribute>
  </xsl:template>
  ...
  <xsl:template name="figure_container_keynote">
    <xsl:attribute name="space-before">.2in</xsl:attribute>
    <xsl:attribute name="space-after">.2in</xsl:attribute>
    <xsl:attribute name="text-align">center</xsl:attribute>
    <xsl:attribute name="font-family"><xsl:value-of select="$main_font"/></xsl:attribute>
  ...
  <xsl:template name="figure_keynote">
  ...
  <xsl:template name="partner_logo_container_intro">
    <xsl:attribute name="font-size">14pt</xsl:attribute>
  ...
  <xsl:template name="supporter_logo_container_intro">
    <xsl:attribute name="font-size">14pt</xsl:attribute>
  ...
  <xsl:template name="partner_logo_intro">
    <xsl:attribute name="content-height">scale-to-fit</xsl:attribute>
    <xsl:attribute name="content-width">7in</xsl:attribute>
    <xsl:attribute name="scaling">non-uniform</xsl:attribute>
    <xsl:attribute name="text-align">center</xsl:attribute>
  ...
  <xsl:template name="supporter_logo_intro">
    <xsl:attribute name="content-height">scale-to-fit</xsl:attribute>
    <xsl:attribute name="content-width">2.3in</xsl:attribute>
    <xsl:attribute name="scaling">non-uniform</xsl:attribute>
    <xsl:attribute name="text-align">center</xsl:attribute>
    <xsl:attribute name="display-align">center</xsl:attribute>
    <xsl:attribute name="font-family"><xsl:value-of select="$main_font"/></xsl:attribute>
  ...
  <xsl:template name="head">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$output = 'pdf'">
        ...
        <!--<xsl:attribute name="color">#ffc000</xsl:attribute><!-\- happy yellow -\->-->
        <xsl:attribute name="color">#2fde00</xsl:attribute> <!-- DH2022 green -->
      </xsl:when>
    </xsl:choose>
    <xsl:attribute name="space-before">.2in</xsl:attribute>
    <xsl:attribute name="space-after">.2in</xsl:attribute>
  ...
  <!-- silly, indeed... added support for DH-Heading3: subsubsubsubsubhead -->
  <xsl:template name="subsubsubsubsubhead">
    <xsl:attribute name="font-family"><xsl:value-of select="$main_font"/></xsl:attribute>
    <xsl:attribute name="font-size">10pt</xsl:attribute> 
    <xsl:attribute name="color">black</xsl:attribute> 
    <xsl:attribute name="space-before">.2in</xsl:attribute>
    <xsl:attribute name="space-after">.15in</xsl:attribute>
  ...
  <xsl:template name="logo_figure_head">
    <!--<xsl:attribute name="font-style">italic</xsl:attribute>-->
    <xsl:attribute name="font-size">12pt</xsl:attribute>
    <xsl:attribute name="font-family"><xsl:value-of select="$main_font"/></xsl:attribute>
    <xsl:attribute name="margin-left">.1in</xsl:attribute>
...
<!-- Spezialfall: Bild in Tabelle -->
  ...
  <!-- Tables -->
    <xsl:template name="table_container">
    <xsl:attribute name="space-before">.2in</xsl:attribute>
    <xsl:attribute name="space-after">.2in</xsl:attribute>
    <xsl:attribute name="font-family"><xsl:value-of select="$main_font"/></xsl:attribute>
  </xsl:template>
  ...
  <xsl:variable name="header_and_footer">
    <fo:static-content flow-name="before-even">
      ...
      <fo:block><xsl:call-template name="page_header"/>Digital Humanities im deutschsprachigen Raum 2022</fo:block>
    </fo:static-content>
 ...
 <!-- <xsl:template match="//term[text()='Keynote']" mode="toc">
  ...
  <xsl:template match="//term[text()='Vortrag']" mode="toc">
    <xsl:if test="(//term[text()='Vortrag'])[1] is ."><fo:block keep-with-next="always"><xsl:call-template name="subhead"/>Vorträge</fo:block></xsl:if>
  </xsl:template>
  
  <xsl:template match="//term[text()='Doctoral Consortium']" mode="toc">
    <xsl:if test="(//term[text()='Doctoral Consortium'])[1] is ."><fo:block keep-with-next="always"><xsl:call-template name="subhead"/>Doctoral Consortium</fo:block></xsl:if>
  </xsl:template>
 
  <xsl:template match="//term[text()='Posterpräsentation']" mode="toc">
    <xsl:if test="(//term[text()='Posterpräsentation'])[1] is ."><fo:block keep-with-next="always"><xsl:call-template name="subhead"/>Posterpräsentationen</fo:block></xsl:if>
  </xsl:template>
  ...
  <xsl:template match="/">
    ...
    <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:fox="http://xmlgraphics.apache.org/fop/extensions">
      ...
      <!-- Beginning image -->
      <!-- FIX: this caused errors, maybe manually add them -->
     ...
     <!-- <xsl:if test="$output = 'pdf'">
        <fo:page-sequence master-reference="Full">
          <fo:flow flow-name="xsl-region-body">
            <fo:block>
              <fo:external-graphic src="Titlepage.pdf"
      ...
      <!-- Here begins the code for the four content sections: Panels, Papers, and Posters.
      ...
      <!-- We changed the original order for DHd2022 in Potsdam from:
      ...
      <!-- doesn't make any difference, if you change the order here... -->
  ...
  <!--      <fo:page-sequence master-reference="Single">
            ...
            <xsl:for-each select="//TEI">
              ...
              <xsl:if test="normalize-space(teiHeader[1]/profileDesc[1]/textClass[1]/keywords[@n='subcategory']/term[1]) = 'Panel'">
                ...
                <fo:block id="{@n}" keep-with-next="always">
                  <xsl:call-template name="head"/>
                  <xsl:choose>
                    <xsl:when test="teiHeader/fileDesc/titleStmt/title/title[@type eq 'main'] and teiHeader/fileDesc/titleStmt/title/title[@type eq 'sub']">
                      <!-- fix main-/subtitle -->
                      <!-- the following code would work and add "–" between main and sub -->
                      <!--<xsl:value-of select="teiHeader/fileDesc/titleStmt/title/title[@type eq 'main']"/>
                      <xsl:text> – </xsl:text>
                      <xsl:value-of select="teiHeader/fileDesc/titleStmt/title/title[@type eq 'sub']"/>-->
                      <xsl:value-of select="teiHeader/fileDesc/titleStmt/title/title[@type eq 'main']"/>
                      <!-- fix with main/sub with additional template -->
                      <fo:block>
     ...
     <!--   <fo:page-sequence master-reference="Single"> 
      ...
      <!-- Title Page -->
      <xsl:if test="//keywords[@n='subcategory']/term[1] = 'Doctoral Consortium'">
  <!--      <fo:page-sequence master-reference="Single">
          <fo:flow flow-name="xsl-region-body">
            <fo:block>
              <xsl:call-template name="section_head"/>
              <xsl:text>Doctoral Consortium</xsl:text>
            </fo:block>
          </fo:flow>
        </fo:page-sequence> -->
        ...
        <fo:page-sequence master-reference="Double">
          ...
          <fo:flow flow-name="xsl-region-body">
            <xsl:for-each select="//TEI">
              <!-- if DocCon. This is a lazy way to select the panels, but I wasn't too concerned with code efficiency. -->
              <xsl:if test="normalize-space(teiHeader[1]/profileDesc[1]/textClass[1]/keywords[@n='subcategory']/term[1]) = 'Doctoral Consortium'">
                <!-- Title -->
                <fo:block id="{@n}" keep-with-next="always">
                  <xsl:call-template name="head"/>
                  <xsl:choose>
      ...
      <!-- Title Page -->
      <xsl:if test="//keywords[@n='subcategory']/term[1] = 'Posterpräsentation'">
   <!--     <fo:page-sequence master-reference="Single">
          <fo:flow flow-name="xsl-region-body">
            <fo:block>
              <xsl:call-template name="section_head"/>
              <xsl:text>Posterpräsentationen</xsl:text>
            </fo:block>
          </fo:flow>
        </fo:page-sequence> -->
        ...
        <fo:page-sequence master-reference="Double">
          ...
          <fo:flow flow-name="xsl-region-body">
            <xsl:for-each select="//TEI">
              ...
              <xsl:if test="normalize-space(teiHeader[1]/profileDesc[1]/textClass[1]/keywords[@n='subcategory']/term[1]) = 'Posterpräsentation'">
                <!-- Title -->
                <fo:block id="{@n}" keep-with-next="always">
                  <xsl:call-template name="head"/>
                  <xsl:choose>
 ...
 <!--       <fo:page-sequence master-reference="Single">
    ...
    <!--  <fo:page-sequence master-reference="Double">
      ...
      <!--<xsl:if test="$output = 'pdf'">
        <fo:page-sequence master-reference="Full">
          <fo:flow flow-name="xsl-region-body">
           <fo:block>
              <fo:external-graphic src="{$backcover}" content-height="297mm" content-width="210mm"/>
            </fo:block>
          </fo:flow>
        </fo:page-sequence>
      </xsl:if>-->

      <!-- Necessary empty pages for printing -->

      <!--<xsl:if test="$output = 'pdf'">
        <fo:page-sequence master-reference="Full">
          <fo:flow flow-name="xsl-region-body">
            <fo:block>
              <fo:external-graphic src="{concat($covers,'#page=2')}"
                                   content-width="scale-to-fit"
                                   content-height="scale-to-fit"
                                   scaling="uniform" />
            </fo:block>
      ...
      <xsl:if test="$output = 'pdf'">
        <fo:page-sequence master-reference="Full">
          <fo:flow flow-name="xsl-region-body">
            <fo:block>
              <fo:external-graphic src="{concat($covers,'#page=3')}"
                                   content-width="scale-to-fit"
                                   content-height="scale-to-fit"
                                   scaling="uniform" />
            </fo:block>
      ...
      <xsl:if test="$output = 'pdf'">
        <fo:page-sequence master-reference="Full">
          <fo:flow flow-name="xsl-region-body">
            <fo:block>
              <fo:external-graphic src="{concat($covers,'#page=4')}"
                                   content-width="scale-to-fit"
                                   content-height="scale-to-fit"
                                   scaling="uniform" />
            </fo:block>
  ...
  <xsl:template match="head">
    <xsl:choose>
      ...
      <!-- fix: add support for DH-Headings3 -->
      <xsl:when test="ancestor::div[@type='div3']">
        <fo:block>
          <xsl:call-template name="subsubsubsubsubhead"/> <!-- 5x sub... -->
          <xsl:apply-templates/>
        </fo:block>
      </xsl:when>
      ...
      <xsl:when test="ancestor::div[@type='div2']">
        <fo:block>
          <xsl:call-template name="subsubsubsubhead"/> <!-- 4x sub... -->
          <xsl:apply-templates/>
        </fo:block>
      </xsl:when>
  ...
  <xsl:template match="div[@type='div1']">
    <fo:block  keep-together.within-page="always"></fo:block>
    <xsl:apply-templates/>
  </xsl:template>
  ...
  <!--<xsl:template match="note">
    <xsl:choose>
      &lt;!&ndash; numbered end notes &ndash;&gt;
      <xsl:when test="@n">
        <fo:block text-align="left">
          <xsl:call-template name="text"/>
  ...
  <xsl:template match="hi">
    <xsl:choose>
      ...
      <xsl:when test="@rend='Arabic'">
        <fo:inline><xsl:call-template name="arabic"/><xsl:apply-templates/></fo:inline>
      </xsl:when>
      ...
      <xsl:when test="@rend='Ogham'">
        <fo:inline ><xsl:call-template name="ogham"/><xsl:apply-templates/></fo:inline>
      </xsl:when>
  ...
  <xsl:template match="table">
    <fo:block keep-together.within-page="always">
      <xsl:call-template name="table_container"/>
      <fo:block text-align="left"><xsl:call-template name="table_head"/><xsl:apply-templates select="head"/></fo:block>
